...
SecureSECO DAO operates a dual-token structure comprising of a governance token (SECOREP) and a monetary/utility token (SECOIN). This unique system brings numerous benefits that cater to our ambition of fostering a decentralized, robust, and transparent ecosystem.
...
SECOREP operates as SecureSECO DAO's governance token. It equips its holders with the power to propose and vote on pivotal decisions that shape the SecureSECO ecosystem.

SECOREP embodies the heart of SecureSECO DAO's decentralized governance model, ensuring the platform remains aligned with its democratic ideals by allowing every token holder a voice in its operations.

## Monetary/Utility token (SECOIN)

SECOIN acts as SecureSECO DAO's monetary and utility token. It is utilized for making payments for queries and rewarding miners for their contributions to the ecosystem.

SECOIN helps cultivate a flourishing economy within the SecureSECO DAO platform. Its design promotes a balance between supply and demand, fostering a sustainable reward mechanism for users who contribute value to the network.
...
<Cards
  cardData={[
    ...
    },
    {
      ...
      description:
        "Explore the function and importance of SECO, SecureSECO DAO's monetary and utility token. Discover how it powers our economy and interacts with the Augmented Bonding Curve.",
      icon: Icons.secoin,
      pattern: {
        y: -6,
        squares: [
