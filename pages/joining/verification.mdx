...
Following successful verification, the user is redirected back to our site, which receives a verification token. This token allows our server to confirm the user's identity. Once confirmed, the server generates a proof which the user can then use to verify their identity on-chain.
