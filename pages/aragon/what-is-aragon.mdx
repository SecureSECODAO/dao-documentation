...
The Aragon SDK, also known as `@aragon/sdk-client`, is designed to provide developers with easy access to high-level interactions with an Aragon DAO. The SDK comprises three different components:
...
## AragonOSx: The Smart Contracts Foundation

Aragon recently introduced [AragonOSx](https://aragon.org/aragonosx), the smart contract foundation for their platform. While it's still in development, Aragon's team recommends new DAOs to be built on this foundation because of its robust and future-proof design.
